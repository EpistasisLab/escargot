...
def retrieve_file_descriptions(chat_id, step_limit = float('inf')):
    ...
    #load all pkl files
    for f in pkl_files:
        with open('logs/'+str(chat_id)+'/'+f, 'rb') as file:
...
def retrieve_plans(chat_id, user_message_id, step_limit=float('inf')):
